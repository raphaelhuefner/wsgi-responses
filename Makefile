...
install: ## Install local, editable version of the wsgi_responses library.
	pip install -e .

test: ## Run pytest.
	pytest --cov=wsgi_responses --verbose --capture=no tests

fix-isort: ## Run isort against python files in the project directory.
	isort -rc --atomic ./wsgi_responses ./tests

fix-yapf: ## Run yapf against any included or newly introduced Python code.
	yapf -i -r -p ./wsgi_responses ./tests
